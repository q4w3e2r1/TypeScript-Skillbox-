...
const cart = () => {
  const totalPriceCart: HTMLElement = document.querySelector(
    '.cartTab .cartPrice'
  )!;
  ...
  const refreshProductCartHTML = (
    ...
    const totalProductsCount: HTMLElement =
    ...
    if (quantity == 1 && action == 0) {
      ...
      totalProductsCount.textContent = `${
        +totalProductsCount.textContent! + 1
      }`;
      ...
      fetchProduct().then((product) => {
        ...
        const totalPrice =
          Number(totalPriceCart.textContent!.replace('₽', '')) + product.price;
      ...
      if (action == 1) {
        // товар +1
        productPrice =
          Number(price.textContent!.replace('₽', '')) / (quantity - 1);
        newPrice = productPrice * quantity;
        totalPrice =
          Number(totalPriceCart.textContent!.replace('₽', '')) + productPrice;
        totalProductsCount.textContent! = `${
          +totalProductsCount.textContent! + 1
        }`;
      } //товар -1
        ...
        newPrice = productPrice * quantity;
        totalPrice =
          Number(totalPriceCart.textContent!.replace('₽', '')) +
          productPrice * -1;
        totalProductsCount.textContent! = `${
          +totalProductsCount.textContent! - 1
        }`;
  ...
  document.addEventListener('click', (e) => {
    ...
    if (
      ...
      buttonClick.classList.contains('minus')
    ) {
  ...
  openModalBtn.addEventListener('click', () => {
    if (totalPriceCart.textContent != '0₽') modal.style.display = 'flex';
  });
